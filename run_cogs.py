...
if __name__ == "__main__":
    ...
    try:
        ...
        cmd.add_argument(
            "--gpu", default=-1, type=int, help="use id of gpu, -1 if cpu."
        )
        cmd.add_argument(
            "--train_batch_size", default=128, type=int, help="training batch size"
        )
        cmd.add_argument(
            "--eval_batch_size", default=128, type=int, help="training batch size"
        )
        cmd.add_argument("--lr", default=0.01, type=float, help="learning rate")
        cmd.add_argument(
            "--data_path", required=True, type=str, help="path to the training corpus"
        )
        cmd.add_argument(
            "--encoder_config_path", type=str, help="path to the encoder config"
        )
        cmd.add_argument(
            "--decoder_config_path", type=str, help="path to the decoder config"
        )
        ...
        cmd.add_argument(
            "--local_rank", default=-1, type=int, help="multi gpu training"
        )
        ...
        cmd.add_argument(
            "--model_data_path", type=str, required=False, default="./model/"
        )
...
for lf in args.lfs.split(";"):
    for seed in args.seeds.split(";"):  # 42, 66, 77, 88, 99
        ...
        encoder_config_filename = (
            "encoder_config_lstm.json"
            if model_name == "ende_lstm"
            else "encoder_config.json"
        )
        decoder_config_filename = (
            "decoder_config_lstm.json"
            if model_name == "ende_lstm"
            else "decoder_config.json"
        )
        ...
        if "participle_verb" in args.data_path:
            ...
            config_decoder = AutoConfig.from_pretrained(
                os.path.join(args.data_path, decoder_config_filename)
                if args.decoder_config_path is None
                else args.decoder_config_path
            )
        else:
            config_encoder = AutoConfig.from_pretrained(
            ...
            config_decoder = AutoConfig.from_pretrained(
                os.path.join(args.model_data_path, decoder_config_filename)
                if args.decoder_config_path is None
                else args.decoder_config_path
            )
        ...
        if args.do_test:
            trainer.model.eval()
            epoch_iterator = tqdm(
                test_dataloader, desc="Iteration", position=0, leave=True
            )
            total_count = 0
            correct_count = 0
    ...
    if args.output_json:
        ...
        with open(
            os.path.join(args.output_dir, f"{data_filename}.json"), "w"
        ) as outfile:
